...
> 不应该与[椭圆曲线](curves.md)的点混淆，椭圆曲线上的点的说法我们也要经常使用，但是不在多项式求值的语境下使用。
...
> $S$ 是一个至少有 $d$ 个元素的集合。如果我们从 $S$ 中随机选择
...
如果两个多项式相同，那么上式会一直成立，而如果两个多项式不相同，那么上式成立的概率至多就是 $\frac{\max(d_1,d_2)}{|S|}$ 。
...
这就是说每个元素都是 $Z_H(X)$ 的根。我们称 $Z_H(X)$ 就是在 $\mathcal{H}$ 上的 **Vanishing 多项式**
